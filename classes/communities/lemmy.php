...
class Lemmy extends Community
{
  // https://enterprise.lemmy.ml/api/v3/site

  // Properties
  public $platform              = 'lemmy';
  ...
  function __construct(
    ...
    else $this->slug = DEFAULT_LEMMY_COMMUNITY;
    if (!empty($instance)) $this->instance = $instance;
    else $this->instance = DEFAULT_LEMMY_INSTANCE;
  ...
  protected function getCommunityInfo() {
    ...
    $info_directory = "communities/lemmy/$this->instance/$this->slug/about";
    ...
    if (!empty($info)) {
      ...
      if ($this->is_instance_valid) {
        ...
        $url = "https://$this->instance/api/v3/community?name=$this->slug";
        $curl_response = curlURL($url);
        ...
        if (!empty($info['community_view']) && empty($info['error'])) {
          ...
          if (!empty($info['error']) && $info['error'] == 'couldnt_find_community') {
            logger()->error("The requested Lemmy community $this->slug does not exist at the $this->instance instance");
          } else {
            logger()->error("Error retrieving data for the requested Lemmy community $this->slug at the $this->instance instance: " . ($info['error'] ?? 'Unknown error'));
          }
          return;
        }
      }
    ...
    if ($this->is_instance_valid && !empty($info['community_view']) && empty($info['error'])) {
      ...
      $this->feed_title         = "Lemmy - " . $this->title;
  ...
  public function getTopPosts($limit, $period = null) {
    ...
    $cache_directory = "communities/lemmy/$this->instance/$this->slug/top_posts";
    ...
    if ($period) {
      ...
      $cache_directory = "communities/lemmy/$this->instance/$this->slug/top_posts_$period";
  ...
  public function getHotPosts($limit, $filter_nsfw = FILTER_NSFW, $blur_nsfw = BLUR_NSFW) {
    ...
    $cache_directory = "communities/lemmy/$this->instance/$this->slug/hot_posts";
    // Listing types: https://github.com/LemmyNet/lemmy-js-client/blob/main/src/types/ListingType.ts
    // Sort types: https://github.com/LemmyNet/lemmy-js-client/blob/main/src/types/SortType.ts
    $url = "https://$this->instance/api/v3/post/list?community_name=$this->slug&limit=$limit&sort=Hot&type_=All";
    ...
    $hot_posts_min = array();
    foreach ($curl_data['posts'] as $post) {
      $post = new \Post\Lemmy($post, $this->instance, $this->slug);
      $hot_posts_min[] = $post;
    }
  ...
  public function getMonthlyAverageTopScore() {
    ...
    $cache_directory = "communities/lemmy/$this->instance/$this->slug/top_posts_month";
