...
class OpenAI extends Summarizer {
  ...
  protected function getProviderSummary(string $content, string $url): array {
    ...
    if(!empty($response['error']['code']) && $response['error']['code'] === 'invalid_api_key') {
      logger()->error("$this->provider_name API key is invalid. You can find your API key at https://platform.openai.com/account/api-keys. The $this->provider_name summarizer will be disabled for this session.");
