...
class Mistral extends Summarizer {

  // Properties
  private ?string $api_key;
  private string  $api_url = 'https://api.mistral.ai/v1/chat/completions';
  ...
  public function __construct() {
    parent::__construct();
    $this->provider_name      = 'Mistral';
    $this->api_key            = MISTRAL_API_KEY;
    $this->model_name         = MISTRAL_API_MODEL;
  ...
  protected function getProviderSummary(string $content, string $url): array {
    ...
    if (!empty($response['usage']['completion_tokens']) &&
    $response['usage']['completion_tokens'] === $this->max_tokens &&
