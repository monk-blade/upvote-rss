...
class Anthropic extends Summarizer {
  ...
  protected function getProviderSummary(string $content, string $url): array {
    ...
    if(!empty($response['error']['message']) && $response['error']['message'] === 'Invalid Anthropic API Key') {
      logger()->error("$this->provider_name API key is invalid. You can find your API key at https://console.anthropic.com/settings/keys. The $this->provider_name summarizer will be disabled for this session.");
