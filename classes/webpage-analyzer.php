...
class WebpageAnalyzer {
  ...
  public function savePropertiesToCache() {
    ...
    logger()->info('Webpage cache updated for ' . $this->url);
  }
  ...
  private function getStatus() {
    ...
    if (empty($status)) {
      ...
      if ($status < 200 || $status >= 300) {
        logger()->log('WebpageAnalyzer: ' . $this->url . ' returned status ' . $status);
      }
    }
  ...
  private function getParsedData() {
    ...
    if (!empty(MERCURY_URL)) {
      if (!remote_file_exists(MERCURY_URL)) {
        logger()->error('Mercury URL ' . MERCURY_URL . ' is not reachable');
      } else {
    ...
    if (
      ...
      if (remote_file_exists(READABILITY_JS_URL) || getHttpStatus(READABILITY_JS_URL) == 400) {
        ...
        logger()->error('ReadabilityJS URL ' . READABILITY_JS_URL . ' is not reachable');
      }
    }
    ...
    if (empty($parsed_webpage['content'])) {
      ...
      $this->savePropertiesToCache();
      logger()->warning('Content for the URL ' . $this->url . ' could not be parsed');
      if ($display_errors) {
  ...
  public function getOGImage() {
    ...
    logger()->info('Trying to get OG image from ' . $this->url);
    $og_image_url = '';
    $parser = new \Parser\ReadabilityPHP($this->url);
    $webpage_contents = $parser->getBrowserlessPage($this->url) ?? curlURL($this->url) ?? '';
    if (!$webpage_contents) {
      logger()->info('Curl response is empty or invalid when trying to get OG image from ' . $this->url);
      return '';
    }
    $dom = new DOMDocument;
    libxml_use_internal_errors(true);
    ...
    if (empty($og_image_url)) {
      logger()->info('No OG image found for ' . $this->url);
      return '';
    }
    $this->og_image_url = $og_image_url;
    logger()->info("OG image found for $this->url: $og_image_url");
    $this->savePropertiesToCache();
    return $og_image_url;
  }
  ...
  public function getSummary() {
    ...
    // Use the SummarizerFactory to generate the summary
    $result = SummarizerFactory::generateSummary($this->content, $this->url);

    if (empty($result['summary'])) {
      $tries_text = $this->summary_tries > 1 ? ' tries' : ' try';
      $this->summary_error = true;
      logger()->warning('Summary for the URL ' . $this->url . ' could not be generated after ' . $this->summary_tries . $tries_text);
      $this->savePropertiesToCache();
      return '';
    }
    ...
    $this->summary_provider   = $result['provider'];
    $this->summary_model      = $result['model'];
    ...
    // Log the successful summary generation with timing
    $time_text = match (true) {
      $this->summary_total_time == 0 => "less than a second",
      $this->summary_total_time == 1 => "1 second",
      default => $this->summary_total_time . " seconds"
    };
    logger()->info("Summary for URL $this->url generated by $this->summary_provider model $this->summary_model in $time_text");
